...
export default function WelfareAllowancePage() {
  const toast = useToast();
  const [settings, setSettings] = useState<AllowanceSettings | null>(null);
  const [welfareRate, setWelfareRate] = useState('');
  ...
  const fetchSettings = useCallback(async (options?: { refetch?: boolean }) => {
    ...
    try {
      ...
      setWelfareRate(String(Number(data.data.welfareRate)));
    } catch (err) {
  ...
  const isDirty = settings ? Number(welfareRate) !== Number(settings.welfareRate) : false;

  const handleSave = async () => {
    const rate = Number(welfareRate);
    if (Number.isNaN(rate) || rate < 0 || rate > 100) {
      toast.error('Welfare rate must be between 0 and 100');
      return;
    }
    ...
    try {
      const { data } = await api.patch<ApiResponse<AllowanceSettings>>('/allowances/settings', {
        welfareRate: rate,
        managementRate: Number(settings?.managementRate ?? 0),
      });
      setSettings(data.data);
      setWelfareRate(String(Number(data.data.welfareRate)));
      toast.success('Welfare allowance rate updated');
    } catch (err) {
      toast.error(getErrorMessage(err));
    } finally {
      setSaving(false);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Welfare Allowance"
        subtitle="Set the default welfare allowance percentage applied to each employee's basic pay"
      ...
      {loading ? (
        ...
        >
          ...
          <div className="mt-4 rounded-xl bg-neutral-50 p-4 text-sm text-neutral-700">
            <p className="font-medium text-neutral-800">How welfare allowance is calculated</p>
            <p className="mt-2">
              At payroll generation time, the welfare allowance amount is computed as{' '}
              <strong>Basic Pay (Dec 2025) × Welfare Rate%</strong>. The resulting amount is added
              to the employee&apos;s net salary. If an employee has a custom rate set on their
              profile, that rate overrides this default.
            </p>
          </div>
