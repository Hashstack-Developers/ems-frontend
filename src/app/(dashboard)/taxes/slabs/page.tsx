...
function parseOptionalMaxSalary(value: string): number | null {
...
function formatMaxSalary(maxSalary: number | string | null | undefined): string {
  if (maxSalary == null || maxSalary === '') return 'Unlimited';
...
export default function TaxesPage() {
  ...
  const slabFieldComparator = (
    ...
    if (key === 'maxSalary') {
      return parseOptionalMaxSalary(String(current)) === parseOptionalMaxSalary(String(original));
    }
    if (key === 'description' || key === 'taxRate' || key === 'fixedTaxAmount') {
  ...
  const handleSlabSubmit = async (e: FormEvent) => {
    ...
    const payload = {
      ...
      maxSalary: parseOptionalMaxSalary(slabForm.maxSalary),
  ...
  const filteredSlabs = useMemo(() => {
    return slabs.filter((slab) => {
      if (statusFilter === 'active' && !slab.isActive) return false;
      if (statusFilter === 'inactive' && slab.isActive) return false;
      return matchesSearch(search, slab.name, slab.description);
    });
  ...
  return (
    <PageContainer fill>
      ...
      <TableFilters
        ...
        searchPlaceholder="Slab name, description…"
      >
      ...
      <div className="scroll-area min-h-0 flex-1 overflow-y-auto pr-1">
        {loading || refetching ? (
          ...
          <EmptyState icon="🔍" title="No matching tax slabs" description="Try adjusting your search or filters." />
        ) : (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-gradient-to-r from-primary-light/40 to-surface px-5 py-4">
                  <div>
                    ...
                    <p className="text-sm text-muted">
                      Annual: {formatCurrency(Number(slab.minSalary))} – {formatMaxSalary(slab.maxSalary)}
                      ...
                      <table className="data-table w-full min-w-[800px]">
                        <thead>
                          <tr>
                            ...
                            {showSubTaxActionsColumn && <Th className="w-[180px]">Actions</Th>}
                          </tr>
                        </thead>
                        <tbody>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted">
                {formatMaxSalary(parseOptionalMaxSalary(slabForm.maxSalary))}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
